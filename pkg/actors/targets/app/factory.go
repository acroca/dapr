...
// implementation. gRPC channels expose an AppCallbackActorsClient; anything
// else falls through to the HTTP callback protocol. Passing a nil channel
// yields an HTTP transport that will error at call time
func newInvoker(ch channel.AppChannel, r resiliency.Provider, actorType string) transport.Invoker {
	if g, ok := ch.(*grpcchannel.Channel); ok {
		if client := g.AppCallbackActorsClient(); client != nil {
			return grpctransport.New(client, r, actorType)
		}
	}
