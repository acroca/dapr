...
// hostGRPCNoActors is the backward-compatibility regression guard: a gRPC
// app that does not implement AppCallbackActors at all. Daprd's
// GetAppConfig receives codes.Unimplemented and must treat the app as
// hosting no actors, exactly as if it had returned an empty entities list.
...
func (m *hostGRPCNoActors) Setup(t *testing.T) []framework.Option {
	// No WithOnGetRegisteredActorsFn option — the default
	// UnimplementedAppCallbackActorsServer returns codes.Unimplemented.
...
func (m *hostGRPCNoActors) Run(t *testing.T, ctx context.Context) {
	...
	// Unimplemented response from the app is not an error.
